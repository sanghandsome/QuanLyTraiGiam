...
<div class="container mt-4">
    <h1 class="mb-4 text-center text-white bg-dark py-3 rounded shadow-sm" style="font-size: 2rem;">Danh sách Nhân Viên</h1>
    ...
    <div class="d-flex justify-content-between mb-4">
        ...
        <form method="GET" action="{{ route('nhan-vien.index') }}" class="d-flex align-items-center">
            <input type="text" name="search" class="form-control me-2" placeholder="Nhập Mã Nhân Viên..." value="{{ request('search') }}">
            ...
            @else
                @foreach ($nhanviens as $nhanvien)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('nhan-vien.show', $nhanvien->MaNhanVien) }}" class="btn btn-info btn-sm">Xem</a>
                            <a href="{{ route('nhan-vien.edit', $nhanvien->MaNhanVien) }}" class="btn btn-warning btn-sm">Sửa</a>
                            <form action="{{ route('nhan-vien.destroy', $nhanvien->MaNhanVien) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                        ...
                        <td>
                            <a href="{{ route('nhan-vien.phong-giam', $nhanvien->MaNhanVien) }}" class="btn btn-secondary text-white shadow-sm border-0">
